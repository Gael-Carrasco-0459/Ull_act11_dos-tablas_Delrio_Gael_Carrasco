...
def crear_proveedores(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('app_proveedores:listar_proveedores')
    else:
        form = ProveedorForm()
...
def editar_proveedor(request, proveedor_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('app_proveedores:detalle_proveedor', proveedor_id=proveedor.id)
    else:
        form = ProveedorForm(instance=proveedor)
    return render(request, 'formulario_proveedor.html', {'form': form, 'titulo': 'Editar Proveedor'})

def borrar_proveedores(request, proveedor_id):
    proveedor = get_object_or_404(Proveedor, id=proveedor_id)
    if request.method == 'POST':
        proveedor.delete()
        return redirect('app_proveedores:listar_proveedores')
